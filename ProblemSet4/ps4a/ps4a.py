...
WORDLIST_FILENAME = "/home/suspect-0/.config/spyder-py3/ProblemSet4/words.txt"
...
def isValidWord(word, hand, wordList):
    ...
    for key in new_hand:
        if hand.get(key, 0) < new_hand.get(key, 0):
            return False
        
    #this one works but the system doesn't support it 
    # for letter in word:
    #     key = new_hand.get(letter, 0)
    #     if key < 0:
    #         return False
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while n:
        ...
        usersWord = input('Enter word, or a "." to indicate that you are finished: ')
        if(usersWord == '.'):
            print('Goodbye! Total score:', total, 'points.')
            break
        elif(not isValidWord(usersWord, hand, wordList)):
            print("Invalid word, please try again.\n")
      
        else:
            point = getWordScore(usersWord, wordLength)
            total += point
            print(usersWord, "earned", point, "points. Total:", total, "points")
